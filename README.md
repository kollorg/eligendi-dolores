# @kollorg/eligendi-dolores&nbsp;&nbsp;&nbsp;[![Build Status](https://github.com/kollorg/eligendi-dolores/workflows/ci/badge.svg)](https://github.com/kollorg/eligendi-dolores/actions) [![Build status](https://github.com/kollorg/eligendi-dolores/workflows/win-ci/badge.svg)](https://github.com/kollorg/eligendi-dolores/actions) [![Coverage Status](https://coveralls.io/repos/davedoesdev/@kollorg/eligendi-dolores/badge.png?branch=master)](https://coveralls.io/r/davedoesdev/@kollorg/eligendi-dolores?branch=master) [![NPM version](https://badge.fury.io/js/@kollorg/eligendi-dolores.png)](http://badge.fury.io/js/@kollorg/eligendi-dolores)
...
- **Note:** An alternative module, [`qlobber-pg`](https://github.com/davedoesdev/qlobber-pg), can be used when you need access from multiple hosts. It's API-compatible with `@kollorg/eligendi-dolores` and requires a PostgreSQL database.
- Highly configurable.
...
```javascript
var QlobberFSQ = require('@kollorg/eligendi-dolores').QlobberFSQ;
...
```javascript
const { QlobberFSQ } = require('@kollorg/eligendi-dolores');
...
```shell
npm install @kollorg/eligendi-dolores
```

## Limitations

- `@kollorg/eligendi-dolores` provides no guarantee that the order messages are given to subscribers is the same as the order in which the messages were written. If you want to maintain message order between readers and writers then you'll need to do it in your application (using ACKs, sliding windows etc). Alternatively, use the `order_by_expiry` [constructor](#qlobberfsqoptions) option to have messages delivered in order of the time they expire.

- `@kollorg/eligendi-dolores` does its best not to lose messages but in exceptional circumstances (e.g. process crash, file system corruption) messages may get dropped. You should design your application to be resilient against dropped messages.

- `@kollorg/eligendi-dolores` makes no assurances about the security or privacy of messages in transit or at rest. It's up to your application to encrypt messages if required.

- `@kollorg/eligendi-dolores` supports Node 6 onwards.

## Distributed filesystems

Note: When using a distributed file system with `@kollorg/eligendi-dolores`, ensure that you synchronize the time and date on all the computers you're using.
...
`@kollorg/eligendi-dolores` has been tested with FraunhoferFS 2014.01 on Ubuntu 14.04 and FraunhoferFS 2012.10 on Ubuntu 13.10.

### CephFS

`@kollorg/eligendi-dolores` has been tested with CephFS 0.80 on Ubuntu 14.04. Note that you'll need to [upgrade your kernel](http://www.yourownlinux.com/2014/04/install-upgrade-to-linux-kernel-3-14-1-in-linux.html) to at least 3.14.1 in order to get the fix for [a bug](http://tracker.ceph.com/issues/7371) in CephFS.

## How it works

![How it works](http://rawgit.davedoesdev.com/davedoesdev/@kollorg/eligendi-dolores/master/diagrams/how_it_works.svg)

Under the directory you specify for `fsq_dir`, `@kollorg/eligendi-dolores` creates the following sub-directories:
...
`@kollorg/eligendi-dolores` reads `UPDATE` at regular intervals to determine whether a new message has been written to a bucket. If it has then it processes each filename in the bucket's directory listing.
...
- The `@kollorg/eligendi-dolores` module is installed at the same location.
...
[c8](https://github.com/bcoe/c8) results are available [here](http://rawgit.davedoesdev.com/davedoesdev/@kollorg/eligendi-dolores/master/coverage/lcov-report/index.html).

Coveralls page is [here](https://coveralls.io/r/davedoesdev/@kollorg/eligendi-dolores).
...
- `{Object} [options]` Configures the file system queue. Valid properties are listed below:
  - `{String} fsq_dir` The path to the file system queue directory. Note that the following sub-directories will be created under this directory if they don't exist: `messages`, `staging`, `topics` and `update`. Defaults to a directory named `fsq` in the `@kollorg/eligendi-dolores` module directory.
  ...
  - `{Integer} poll_interval` `@kollorg/eligendi-dolores` reads the `UPDATE` file at regular intervals to check whether any messages have been written. `poll_interval` is the time (in milliseconds) between each check. Defaults to 1 second.
